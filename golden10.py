...
win_width, win_height = 1250, 950

PLAYERS = 4
SUITS = ['black', 'red', 'green', 'yellow']

HAND_POSITIONS = [[(50*i+250, 0, 0) for i in range(14)],
                [(0, -50*i+win_height-100, 0) for i in range(14)],
                [(-50*i+win_width-250, win_height, 0) for i in range(14)],
                [(win_width, 50*i+win_height-850, 0) for i in range(14)]]

DISCARDS = [(550,250,0), (350,400,0), (550,510,0), (750,410,0)]

window = pyglet.window.Window(win_width,win_height,"Golden 10")
...
class Golden10Game(object):
    ...
    def count_cards(self):
        for i in range(PLAYERS):
            round_score = 0
            for j in range(len(g.won_tricks[i])):
                t_card = g.won_tricks[i][j]
                if t_card.golden10_value() != 0:
                    # t_card.sprite.visible = True
                    self.players[i].append(t_card)
                print(f"    {t_card} - {t_card.golden10_value()} points")
                round_score += t_card.golden10_value()
            print(f"Player {i} - {self.score[i]} points\n")
            self.score[i] = max(self.score[i]+round_score, 0)
        window.set_caption(f"Current Scores: {g.score}")
    ...
    def trick_winner(self):
        ...
        winner = self.trick.index(max(follow_suit))
...
def selected_card(x,y):
    for i in range(len(g.players[0])):
        ...
        if x>cx and x<cx+(135 if i == len(g.players[0])-1 else 50) and y>0 and y<210:
            if (g.led_suit == '') or (not can_follow_suit()) or (can_follow_suit() and c.suit == g.led_suit):
                return c, i
